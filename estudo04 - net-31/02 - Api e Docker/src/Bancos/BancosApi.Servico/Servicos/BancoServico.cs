...
namespace BancosAPI.Servico.Servicos
{
    ...
    public class BancoServico : IBancoServico
    {
        ...
        //}
        public async Task<Resultado<BancoResposta>> GetID(string id)
        {
            var resultado = new Resultado<BancoResposta>();

            var existente = await _repositorio.GetID(id);
            if (existente == null)
            {
                return resultado.Falha($"Registro não encontrado: {id}", true);
            }

            var resposta = _mapper.Map<BancoResposta>(existente);
